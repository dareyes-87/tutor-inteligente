...
`app/modules/admin/` gestiona la **estructura** del colegio (NO sube libros ni ve el detalle de cada estudiante; eso es del docente). Todos los endpoints usan `require_role(RolUsuario.administrador)`. Cubre CRUD de **docentes** y **estudiantes** (con soft-delete vía `Usuario.activo` — el login ya rechaza inactivos, no hizo falta tocar auth), **grados** y **asignaturas** (DELETE devuelve **409** si tienen dependencias), reset de contraseña, **importación CSV** de estudiantes (`nombre,apellido,grado_id` → auto-genera username `inicial+apellido` y password `apellido+123`, devueltos UNA sola vez), **promoción de año** (`POST /admin/promover-grado` mueve todos los estudiantes activos de un grado a otro) y un **dashboard** global. No requirió migración (todos los campos ya existían). `Grado.nivel` es NOT NULL y se **deriva del nombre** al crear. En el web, `app/admin/` replica la estructura del docente con sidebar slate (`AdminNav`) y 5 páginas (dashboard, docentes, estudiantes, grados, asignaturas) con modales.

### Protección de rutas y redirección por rol (web)

`RequireAuth` (en `lib/auth.tsx`) acepta `roles?: Rol[]`: si el rol del usuario no está permitido, lo redirige a su área con `homeForRole` (estudiante→`/inicio`, docente→`/docente`, administrador→`/admin`). Aplicado en los layouts: `/admin/*` (solo administrador), `/docente/*` (docente + administrador), rutas del estudiante (solo estudiante). El login (`app/login/page.tsx`) ya redirige con `homeForRole` tras autenticar.
